...
function Cover({ book, small = false }: { book: Book; small?: boolean }) {
  return <BookCover book={book} small={small} />;
}
  ...
  return <main className="min-h-screen bg-paper"><Header back="/" /><section className="mx-auto max-w-7xl px-5 pb-20 pt-8 sm:px-8">
    ...
    <div className="mt-7 grid gap-5">{books.map((book, index) => <article key={book.id} className="result-card grid gap-6 rounded-lg border border-border bg-background p-5 transition hover:-translate-y-0.5 hover:shadow-warm-lg sm:grid-cols-[128px_1fr_auto] sm:p-6">
      ...
      <div className="mt-5 grid gap-2">{book.chapters.map((chapter) => <div key={chapter.number} className="flex flex-wrap items-center justify-between gap-2 text-sm">
        <button type="button" onClick={(e) => { e.stopPropagation(); choose(book, chapter); }} className="min-w-0 rounded-sm text-left transition hover:text-primary focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-primary"><span className="underline decoration-transparent underline-offset-4 transition hover:decoration-primary"><strong>Chapter {chapter.number}</strong> · {chapter.title}</span></button>
        <span className="flex shrink-0 items-center gap-3"><span className="text-muted-foreground">{chapter.minutes} min · pp. {chapter.pages}</span>
      ...
      <p className="mt-5 text-xs font-bold uppercase text-muted-foreground">Topics covered</p><div className="mt-2 flex flex-wrap gap-2">{book.topics.filter((topic) => topic.covered).map((topic) => <span key={topic.label} className="rounded-full bg-success-soft px-2.5 py-1 text-xs font-medium text-success"><span aria-hidden="true">✓</span> {topic.label}</span>)}{book.topics.filter((topic) => !topic.covered).slice(0, 2).map((topic) => <span key={topic.label} className="rounded-full bg-muted px-2.5 py-1 text-xs font-medium text-muted-foreground"><span aria-hidden="true">×</span> {topic.label}</span>)}</div>
      <div className="mt-3 flex flex-wrap gap-2">{book.tags.map((tag) => { const hit = topics.some((t) => t.toLowerCase().includes(tag.toLowerCase()) || tag.toLowerCase().includes(t.toLowerCase())); return <span key={tag} className={cn("rounded-full border px-2.5 py-1 text-xs", hit ? "border-primary text-primary" : "border-border text-muted-foreground")}>{tag}</span>; })}</div></div>
      <div className="flex min-w-44 flex-col justify-between gap-5 border-t border-border pt-5 sm:border-l sm:border-t-0 sm:pl-6 sm:pt-0"><p className="flex items-center gap-2 text-sm text-muted-foreground"><Clock3 size={17}/> {bookMinutes(book)} min reading</p><div className="grid gap-2"><Button disabled={!book.chapters.length} onClick={() => choose(book)}>Preview chapter <ArrowRight size={17}/></Button><Button variant="outline" disabled={!book.chapters.length} onClick={() => addToPlan(book)}>{plan.includes(book.id) ? "Remove from plan" : "Add to revision plan"}</Button><a href={book.googleBooksPage} target="_blank" rel="noreferrer" onClick={(e) => e.stopPropagation()} className="text-center text-sm font-semibold text-primary underline-offset-4 hover:underline">View on Google Books</a></div></div>
    </article>)}</div></>}
  </section></main>;
}
...
export function SignupPage() {
  const state = useOnboarding();
  const { selectedBook: book, selectedChapter: chapter, provider, setProvider, payment, setPayment, setSignedUp, addBook, addReading } = state;
